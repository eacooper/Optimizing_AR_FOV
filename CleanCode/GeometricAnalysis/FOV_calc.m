...
for D = 0.1:0.1:4 %diopters
    ...
    for asym_size = [-10 0 10] %amount of asymmetry between left and right half of Mf:nasal angle - temp angle)
        ...
        nearpt = tand(u) * (IPD/2);
        if asym_size == 0
            data_sym(n,:) = [D d T_fov B_reg propBino nearpt];
        elseif asym_size>0 %nasal - temp > 0 --> nasal shift
            data_conv(n,:) = [D d T_fov B_reg propBino nearpt];
        elseif asym_size<0 %nasal - temp < 0 --> temporal shift
            data_div(n,:) = [D d T_fov B_reg propBino nearpt];
        end
...
%put binocular size and FOV in the same plot
